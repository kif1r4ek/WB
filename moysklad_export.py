...
def main():
    # 1. Find "Тест" subfolder under "Дом и красота"
    r = requests.get(f"{BASE}/entity/productfolder",
    ...
    # 2. Fetch all products via assortment endpoint (with pagination)
    products = []
    offset = 0
    limit = 1000
    while True:
        r = requests.get(f"{BASE}/entity/assortment",
    ...
    # Set barcodes column (D) to text format to preserve leading zeros
